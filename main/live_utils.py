...
#relativeTheta / relativeAlpha) * relativeDelta
def get_combined_bands(powers):
    c_powers = np.sum(powers,axis=1)/4
    return c_powers

def compute_concentration(powers):
    a = float((powers[1]*powers[0])/(powers[2]))
    return strip(a)
    #for i in range(4):
    #    concentration_scores[i] = (powers[1,i]*powers[0,i])/(powers[2,i])

def concentration_incrementer(threshold,input):
    if input >= threshold:
        return 2
    else:
        return -2
